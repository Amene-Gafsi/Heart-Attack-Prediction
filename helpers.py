...
def calculate_gradient(y, tx, w):
    ...
    gradient = tx.T.dot(sig - y) / N
    return gradient


def compute_reg_logistic_regression_loss(y, tx, w, lambda_):
    """Compute the regularized loss using negative log-likelihood"""
    pred = sigmoid(tx @ w)
    # We added 1e-8 so that we avoid very small values for the log close to 0
    loss = -np.mean(y * np.log(pred + 1e-8) + (1 - y) * np.log(1 - pred + 1e-8))
    # L2 norm
    reg_term = (lambda_ / 2) * w.T.dot(w)  ## reg term not inside the loss
    return loss


def compute_reg_logistic_regression_gradient(y, tx, w, lambda_):
    """Compute the regularized gradient of the loss function"""
...
def predict(w, data, threshold=Threshold):
    """Predicts the labels of the data using the sigmoid function.
    Args:
        w: weights.
